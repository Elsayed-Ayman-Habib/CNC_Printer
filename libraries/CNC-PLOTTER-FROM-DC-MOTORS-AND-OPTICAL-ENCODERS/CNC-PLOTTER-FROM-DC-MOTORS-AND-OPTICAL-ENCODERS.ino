...
double KP_Y = 60.0;      // P for Y motor
...
void loop()
{
  ...
  input_1 = ((INPUT_X)/(600/200));
  
  INPUT_Y = YEncoder.read();
  input_2 = ((INPUT_Y)/(600/200));
  
//  Serial.print("Encoder X = ");
//  Serial.println(INPUT_X);
  
  // Calculating the error
  ERROR_X = (input_1 - SETPOINT_X);
  ERROR_Y = (input_2 - SETPOINT_Y);
  Serial.print("Error X = ");
  Serial.println(ERROR_X);
  // For debugging
 if (DEBUG_X)
 {
  ACTUAL_X_MM = input_1 / STEPSPERMM_X;
 ...
 if (DEBUG_Y)
 {
    ACTUAL_Y_MM = input_2 / STEPSPERMM_Y;
